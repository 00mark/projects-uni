...
public class Fraction extends Number{
    ...
    private static HashMap<String, Fraction> fracPool = new HashMap<String, Fraction>();
     ...
     * @throws IllegalArgumentException Falls die Eingabe nicht dem regulaeren Ausdruck
     *          entspricht
     * @return Die Fraction Instanz
     */
    public static Fraction parseFraction(String frac) throws IllegalArgumentException{
        // String der Form "Zaehler/Nenner"
        if(frac.matches(FRACREGEX)){
            ...
            return parseFraction(n, d);
        }
        // String der From "Zaehler"
        if(frac.matches(NUMBERREGEX)){
            // Zaehler = Eingabe, Nenner = 1
            int n = Integer.parseInt(frac);
            return parseFraction(n, 1);
        }
        throw new IllegalArgumentException("Ungueltige Eingabe");
    }

    /**
     * Erstellt eine Fraction Instanz aus dem eingegebenen Zaehler
     * @param n Zaehler des Bruchs
     * @return Die Fraction Instanz
     */
    ...
    /**
     * Erstellt eine Fraction Instanz aus dem eingegebenen Zaehler und Nenner
     * @param n Zaehler des Bruchs
     * @param d Nenner des Bruchs
     * @throws ArithmeticException bei Nenner = 0
     * @return die Fraction Instanz
     */
    public static Fraction parseFraction(int n, int d) throws ArithmeticException {
        if(d == 0)
            throw new ArithmeticException("Division durch 0");
        int teiler = ggt(n, d);
        String fraction = (n / teiler) + "/" + (d / teiler);
        // Falls schon eine Fraction Instanz mit den gleichen Werten existiert,
        // gib diese wieder
        if(fracPool.containsKey(fraction))
            return fracPool.get(fraction);
        return new Fraction(n / teiler, d / teiler);
    }
